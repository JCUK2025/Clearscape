...
// Auto-enter if data exists or returning from history/graphs
window.addEventListener('DOMContentLoaded', () => {
  const data = JSON.parse(localStorage.getItem('clearscape-data') || '[]');
  if (data.length > 0 || location.hash === '#dashboard') enterApp();
});

// Open native date picker
function openDatePicker() {
  document.getElementById('date').showPicker?.() || document.getElementById('date').focus();
}
