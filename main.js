  ...
  window.pieChart = new Chart(pieCtx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: ['#a5d6a7','#81c784','#66bb6a','#4caf50','#388e3c','#2e7d32','#c8e6c9','#aed581','#9ccc65','#8bc34a','#7cb342','#689f38']
      }]
    },
    options: {
...
function addRecurringEntries() {
  ...
  const recurring = [
    { type: "Expense", category: "Rent", amount: 500, note: "Monthly rent" },
    { type: "Expense", category: "Wages", amount: 1200, note: "Monthly wages" },
    { type: "Expense", category: "Insurance", amount: 150, note: "Monthly insurance" }
  ];

  const alreadyAdded = transactions.some(t => t.date === today && t.note === "Monthly rent");
  if (!alreadyAdded) {
    recurring.forEach(r => transactions.push({ date: today, ...r, client: "" }));
    localStorage.setItem('clearscapeData', JSON.stringify(transactions));
  }
}
