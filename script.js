...
function updateDashboard() {
    ...
    transactions.forEach(t => {
        ...
        if (new Date(t.date).getFullYear() === currentYear) {
            if (t.type === 'Sale') {
                totalSalesYTD += amountPaid;
            } 
            else if (['Expense', 'Refund'].includes(t.type)) {
                totalOutflowYTD += amountPaid;
            }
        }
    ...
    document.querySelector('#outflowCard .card-details').innerHTML = `<p><span class="detail-label">Transactions (YTD):</span> <span class="detail-value">${transactions.filter(t => ['Expense', 'Refund'].includes(t.type) && new Date(t.date).getFullYear() === currentYear).length}</span></p>`;
