...
function renderDataCentre() {
    ...
    transactions.forEach(t => {
        ...
        if (isRevenue) currentBalance += amountPaid;
        else currentBalance -= amountPaid;

        if (t.type === 'Sale') totalOutstandingAR += (amountDue - amountPaid);

        if (tDate.getFullYear() === currentYear) {
            if (isRevenue) ytdRevenue += amountPaid;
            else ytdExpenses += amountPaid;
        }
        if (tDate.getFullYear() === currentYear && tDate.getMonth() === currentMonth) {
            if (isRevenue) monthRevenue += amountPaid;
            else monthExpenses += amountPaid;
        }
    });
    ...
    // We update hidden summary card elements for use by other parts of the app
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back Buttons
    document.getElementById('backToDashboardBtn').addEventListener('click', () => { renderDataCentre(); showView('dataCentreView'); });
    document.getElementById('backToDashboardFromSettingsBtn').addEventListener('click', () => { renderDataCentre(); showView('dataCentreView'); });
    document.getElementById('backToDashboardFromCategoryBtn').addEventListener('click', () => { renderDataCentre(); showView('dataCentreView'); });
    document.getElementById('backToDashboardFromAnalyticsBtn').addEventListener('click', () => { renderDataCentre(); showView('dataCentreView'); });
    
    // Interactive Dashboard Card (Now on Data Centre Page)
    // You can add listeners to the new cards here if needed
