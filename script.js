...
function updateJobTypeDropdown() {
    ...
    if (selectedType === 'Sale') options = structuredCategories.Sales;
        ...
        options = [...new Set(Object.values(structuredCategories).flat())];
    }
...
function renderNewTransactionPage() {
    document.getElementById('currentBalanceDisplay').textContent = document.getElementById('outstandingBalance').textContent;
...
function updateDashboard() {
    ...
    document.getElementById('totalOutflow').textContent = formatCurrency(totalExpenses);
    document.getElementById('totalOutstanding').textContent = formatCurrency(totalOutstandingAR);
    document.getElementById('outstandingBalance').textContent = formatCurrency(currentBalance);
    document.getElementById('totalOutstanding').className = `amount ${totalOutstandingAR > 0.01 ? 'negative' : 'positive'}`;
    document.getElementById('outstandingBalance').className = `amount ${currentBalance >= 0 ? 'positive' : 'negative'}`;
...
function renderDataCentre() {
    ...
    transactions.forEach(t => {
        ...
        const isRevenue = ['Sale', 'Deposit'].includes(t.type);
        if (tDate.getFullYear() === currentYear) {
            if (isRevenue) ytdRevenue += amount;
            else ytdExpenses += amount;
        }
        if (tDate.getFullYear() === currentYear && tDate.getMonth() === currentMonth) {
            if (isRevenue) monthRevenue += amount;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Main Navigation
    document.getElementById('goToDashboardBtn').addEventListener('click', () => { showView('newTransactionView'); renderNewTransactionPage(); });
    ...
    document.getElementById('headerDashboardBtn').addEventListener('click', () => { showView('newTransactionView'); renderNewTransactionPage(); });
    ...
    ['backToDashboardBtn', 'backToDashboardFromSettingsBtn', 'backToDashboardFromCategoryBtn', 'backToDashboardFromAnalyticsBtn'].forEach(id => {
        ...
        if(btn) btn.addEventListener('click', () => { renderDataCentre(); showView('dataCentreView'); });
    });
