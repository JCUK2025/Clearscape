...
function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(() => {
        sessionStorage.removeItem('isLoggedIn'); // Clear login state on timeout
        showSplashScreen();
    }, 5 * 60 * 1000); // 5 minutes
}
...
function showSplashScreen() {
    ...
    if (factElement) {
        const facts = [
            "Belper is home to the North Mill, a key site in the Industrial Revolution and part of the Derwent Valley Mills World Heritage Site.",
            "The town's name, Belper, is thought to mean 'beautiful retreat' from Norman French.",
            "Famous actor Alan Bates was born in Allestree, Derby, but grew up in Belper.",
            "The River Derwent flows through Belper, once powering the historic mills that shaped the town.",
            "The annual Belper Arts Festival is a major cultural event in the town."
        ];
...
function updateDashboard() {
    ...
    dashboardCardOrder.forEach(cardId => {
        if (cards[cardId]) {
            container.appendChild(cards[cardId]); // Append cards in the correct order
        }
    });
...
function displaySmartInsights() {
    ...
    let insights = [];
    const insightGenerators = [generateOverdueInsights, generateSpendingInsights, generateExpenseTrendInsight, generateTopServiceLastYear, generateTopClientInsight, generateUpcomingRecurringInsights, generateQuoteInsight];
...
function generatePdf(transactionId) {
    ...
    html2canvas(invoiceContainer).then(canvas => {
        ...
        pdf.save(`${transaction.transactionNumber || 'invoice'}_${transaction.name}.pdf`);
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back Buttons
    ['backToDashboardBtn', 'backToDashboardFromSettingsBtn', 'backToDashboardFromCategoryBtn', 'backToDashboardFromAnalyticsBtn', 'backToDashboardFromSmartAnalysisBtn', 'backToDashboardFromCalendarBtn'].forEach(id => {
    ...
    function updateTransactionNumberDisplay() {
        ...
        if (type === 'Sale') { displayEl.value = `INV-${nextInvoiceNumber}`; }
        else if (type === 'Expense') { displayEl.value = `EXP-${nextExpenseNumber}`; }
        else { displayEl.value = 'N/A'; }
    }
    ...
    function populateSettingsPage() {
        ...
        document.getElementById('companyAddressSetting').value = savedDetails.address || '';
        document.getElementById('companyPhoneSetting').value = savedDetails.phone || '';
        document.getElementById('companyEmailSetting').value = savedDetails.email || '';
        document.getElementById('companyWebsiteSetting').value = savedDetails.website || '';
        renderClientList();
    }
    ...
    // Client Management & Category Management will be called from settings unlock/navigation
