...
function renderAnalyticsPage(year) {
    ...
    transactions.forEach(t => {
        ...
        if (new Date(t.date).getFullYear() === selectedYear) {
            if (t.type === 'Sale') totalSalesYTD += amountPaid;
            else if (['Expense', 'Refund'].includes(t.type)) totalOutflowYTD += amountPaid;
        }
        
        if (t.type === 'Sale') totalOutstandingAR += (amountDue - amountPaid);
    });

    document.getElementById('totalSales').textContent = formatCurrency(totalSalesYTD);
    document.getElementById('totalOutflow').textContent = formatCurrency(totalOutflowYTD);
...
function renderSmartAnalysisPage() {
    ...
    yearSelect.removeEventListener('change', updateMonthlyCharts);
    monthSelect.removeEventListener('change', updateMonthlyCharts);
...
function updateMonthlyCharts() {
    ...
    const yearData = processDataForYear(year);
