...
function renderFullTransactionList() {
    ...
    // Add more filter logic here later if needed (e.g., dates)
    ...
    const filteredTransactions = transactions.filter(t => {
        ...
        if (filterPaidStatusValue === 'Outstanding') {
            matchesPaidStatus = (parseFloat(t.amountDue) - parseFloat(t.amountPaid)) > 0.01;
    ...
    filteredTransactions.forEach(t => {
        ...
        item.innerHTML = `
            <div class="transaction-summary">
                ...
                <span class="transaction-amount" style="color: ${t.type === 'Sale' || t.type === 'Deposit' ? 'var(--positive-color)' : 'var(--negative-color)'};">
            ...
            <div class="transaction-details">
                ...
                <p><strong>Amount Paid:</strong> ${formatCurrency(parseFloat(t.amountPaid))}</p>
        ...
        // Add click listener to toggle details view
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW LOGIC FOR FORMS AND BUTTONS ---

    // 1. Transaction Form Submission
    document.getElementById('transactionForm').addEventListener('submit', function(event) {
        event.preventDefault();

        const newTransaction = {
            id: 't_' + Date.now(), // Unique ID
            ...
            isRecurring: document.getElementById('isRecurringCheckbox').checked
        };

        if (!newTransaction.date || !newTransaction.name || !newTransaction.amountDue) {
            alert('Please fill out Date, Name, and Amount Due.');
            return;
        }
    ...
    document.getElementById('unlockSettingsBtn').addEventListener('click', () => {
        ...
        const unlockView = document.getElementById('settingsUnlockView');
        const settingsContent = document.getElementById('actualSettings');
    ...
    function exportToCsv(filename, rows) {
        const processRow = row => row.map(val => {
            let finalVal = val.toString().replace(/"/g, '""');
    ...
    document.getElementById('exportCsvBtn').addEventListener('click', () => {
        ...
        exportToCsv("ClearScape_Transactions.csv", [headers, ...rows]);
    });
