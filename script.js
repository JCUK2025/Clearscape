...
function renderDataCentre() {
    ...
    transactions.forEach(t => {
        ...
        if (isRevenue) currentBalance += amount;
    ...
    // Update the main balance display from the old dashboard view, which is now hidden
    // This element is still in the DOM and useful for this calculation
    const outstandingBalanceEl = document.getElementById('outstandingBalance');
    if (outstandingBalanceEl) outstandingBalanceEl.textContent = formatCurrency(currentBalance);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back Buttons
    ['backToNewTransactionBtn', 'backToDashboardFromSettingsBtn', 'backToDashboardFromCategoryBtn', 'backToDashboardFromAnalyticsBtn'].forEach(id => {
    ...
    // We'll need to re-add this if you want to click the YTD/Monthly cards
