...
function renderHistory(filtered = data) {
  ...
  filtered.forEach((tx, index) => {
    ...
    entry.innerHTML = `
      <strong>${tx.date}</strong> – ${tx.type.toUpperCase()}<br>
      ...
      <div class="history-actions">
        ...
        ${tx.type === 'sale' && tx.amountPaid < tx.amountDue ? `<button onclick="addPayment(${index})">💳 Add Payment</button>` : ''}
      </div>
    `;
...
function addPayment(index) {
  ...
  const add = prompt(`Add payment (up to £${remaining.toFixed(2)}):`, '');
  if (add !== null) {
